...
fn widget_time(time: &Time) -> iced::Element<'static, MessageDispatcher> {
    ...
    let mut saldo_indicacoes: f32 = 0.0;
    let mut saldo_retirada: f32 = 0.0;
    ...
    for movimentacao in time.financeiro.obter_movimentacoes() {
        match movimentacao.tipo {
            ...
            TipoMovimentacao::Indicacao => saldo_indicacoes += movimentacao.valor as f32 / 100.0,
            TipoMovimentacao::Retirada => saldo_retirada += movimentacao.valor as f32 / 100.0,
            _ => {}
        }
    }

    let saldo_total = time.financeiro.obter_saldo();

    let lista = column![
        row![
            text("Participações").font(segoe_ui).size(13),
            Space::with_width(Length::Fill),
            text("R$").font(segoe_ui).size(13),
            text(format!("{:.0}", saldo_participacao)).align_x(Alignment::End).width(40).font(segoe_ui).size(13),
        ],
        // Space::with_height(Length::Fill),
        row![
            text("Depósitos").font(segoe_ui).size(13),
            Space::with_width(Length::Fill),
            text("R$").font(segoe_ui).size(13),
            text(format!("{:.0}", saldo_deposito)).align_x(Alignment::End).width(40).font(segoe_ui).size(13),
        ],
        // Space::with_height(Length::Fill),
        row![
            text("Indicações").font(segoe_ui).size(13),
            Space::with_width(Length::Fill),
            text("R$").font(segoe_ui).size(13),
            text(format!("{:.0}", saldo_indicacoes)).align_x(Alignment::End).width(40).font(segoe_ui).size(13),
        ],
        // Space::with_height(Length::Fill),
        row![
            text("Premiações").font(segoe_ui).size(13),
            Space::with_width(Length::Fill),
            text("R$").font(segoe_ui).size(13),
            text(format!("{:.0}", saldo_premiacao)).align_x(Alignment::End).width(40).font(segoe_ui).size(13),
        ],
        // Space::with_height(Length::Fill),
        row![
            text("Retirada").font(segoe_ui).size(13),
            Space::with_width(Length::Fill),
            text("R$").font(segoe_ui).size(13),
            text(format!("{:.0}", saldo_retirada)).align_x(Alignment::End).width(40).font(segoe_ui).size(13),
        ],
    ];
    ...
    column![
        row![
            ...
            column![
                text(format!("{}", time.nome_do_time)).font(segoe_ui_bold).size(18),
                row![
                    text(format!("R$ {:.0}", saldo_total))
                        .font(segoe_ui_bold)
                        .size(20)
                        .style(move |_: &iced::Theme| text::Style {
                            color: Some(if saldo_total < 0.0 {
                    ...
                    Space::with_width(Length::Fill),
                    text(format!("{}", time.nome_do_dono)).font(segoe_ui).size(13),
                ].width(Length::Fill)
            ]
        ].spacing(5).padding(5),
        lista.padding(5)
